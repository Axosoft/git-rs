...
use futures::sync::mpsc::{unbounded as channel, UnboundedSender as Sender, UnboundedReceiver as Receiver};
...
impl ClientHandler {
    fn new(state: Arc<Mutex<SharedState>>, socket: TcpStream) -> Self {
        ...
        state.lock().unwrap().channel_by_id.insert(uuid, sender.clone());
...
pub fn handle_client(
    ...
    let connection = io::write_all(client_handler.socket, hello_message)
        .then(|result| {
